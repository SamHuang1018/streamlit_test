...
def init_database():
    ...
    # Migrate: Add accumulated_prior_cost column to reports if not exists
    try:
    ...
    if reference_floor:
        ...
        if submitted:
            if not workers.strip():
                ...
                # ===== 施工中/施工完成利潤邏輯 =====
                # 判斷是否有任何「施工完成」的標準項目（qty > 0）
                has_completed_item = any(
                    item['quantity'] > 0 and not item.get('is_custom', False)
                    for item in selected_items
                )
                current_edit_report_id = report_id if is_editing else None

                # 查詢所有施工完成項目在過去施工中記錄的累積工+料成本
                ...
                # 施工中：profit=0（成本已記錄，但尚未完工不計利潤）
                # 施工完成：profit = 產值 - 今日成本 - 先前所有施工中累積成本
                if has_completed_item:
                    profit = total_revenue - total_cost - total_accumulated_prior
                else:
                    profit = 0
                ...
                if current_role == 'admin':
                    ...
                    col_s2.metric("今日成本", f"${total_cost:,.0f}")
                    if total_accumulated_prior > 0:
                        col_s3.metric("累積前期成本", f"${total_accumulated_prior:,.0f}", help="施工中各天的工+料成本合計")
                    else:
                        col_s3.metric("累積前期成本", "$0")
        ...
        with tab2:
            ...
            if not df.empty:
                ...
                if floor_data:
                    ...
                    # 狀態：只要 MAX(qty) > 0 即代表已完成（不管之前有多少天施工中記錄）
                    floor_df['狀態'] = floor_df.apply(
                        lambda x: "✅ 已完成" if x['數量(MAX)'] > 0 else "🔄 施工中",
                        axis=1
                    )
        ...
        with tab_analysis:
            ...
            if ana_project:
                ...
                if ana_data:
                    ...
                    st.dataframe(
                        ana_df[['樓層', '項目名稱', '單位', '數量', '累計工數', '累計產值', '預估人力成本', '項目利潤', '利潤率', '平均工率', '施工天數', '狀態']],
                        column_config={
