...
def init_database():
    ...
    try:
        c.execute('ALTER TABLE reports ADD COLUMN accumulated_prior_cost REAL DEFAULT 0')
    except sqlite3.OperationalError:
        pass
...
def save_report(report_data, items, materials, photos=None):
    ...
    c.execute('''
        INSERT INTO reports (report_id, date, project_name, building_name, floor_name, workers,
                           ...
                           revenue, material_cost, total_cost, profit, efficiency, total_quantity, accumulated_prior_cost)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ''', (
        ...
        report_data['efficiency'], total_quantity, report_data.get('accumulated_prior_cost', 0)
    ))
    for item in items:
...
def update_report(report_id, report_data, items, materials, photos=None):
    ...
    c.execute('''
        ...
        SET date = ?, building_name = ?, floor_name = ?, workers = ?,
            ...
            efficiency = ?, total_quantity = ?, accumulated_prior_cost = ?, updated_at = CURRENT_TIMESTAMP
        WHERE report_id = ?
    ''', (
        ...
        report_data['material_cost'], report_data['total_cost'], report_data['profit'],
        report_data['efficiency'], total_quantity, report_data.get('accumulated_prior_cost', 0), report_id
    ))
    c.execute('DELETE FROM report_items WHERE report_id = ?', (report_id,))
    c.execute('DELETE FROM report_materials WHERE report_id = ?', (report_id,))
    for item in items:
...
def render_report_form(is_editing, edit_data=None):
    ...
    渲染日報表單 (v7 修正版)
    v7 核心修正：
    - [BUG修正] 利潤計算只扣「完成項目」的工數成本，不扣「施工中」項目的工數
    - [BUG修正] has_completed_item 同時檢查標準項目和自訂項目
    - [BUG修正] 工率計算用完成項目的工數作分母
    """
    ...
    with col2:
        progress_desc = st.text_area("施工進度描述", 
            value=edit_desc if is_editing else "", height=200,
            placeholder="例如：\n- A棟2F浴室防水完成\n- 素地整理完成\n- 材料搬運")
    ...
    if reference_floor:
            ...
            if is_editing and not edit_items_df.empty:
                ...
                for _, item_row in std_items_df.iterrows():
                    edit_items_dict[item_row['項目名稱']] = {
                        'quantity': item_row['數量'], 'completion_days': item_row['完成天數'], 'worker_count': item_row['計工']}
            ...
            for item_name, std_qty, unit, unit_price in floor_items:
                ...
                if current_role == 'admin':
                    ...
                    with col_check:
                        ...
                        checked = st.checkbox(label, value=default_checked or item_is_locked, key=f"item_{item_name}_{report_id}", disabled=item_is_locked)
                    if checked or item_is_locked:
                        ...
                        with col_qty:
                            if item_is_locked:
                                ...
                                qty = st.number_input(f"數量({unit})", value=qty_val, step=0.5, disabled=is_prep, key=f"qty_{item_name}_{report_id}", label_visibility="collapsed")
                        with col_workers:
                            if item_is_locked:
                                st.write(f"{default_workers}"); item_workers = default_workers
                            else:
                                item_workers = st.number_input("計工", value=float(default_workers), step=0.5, key=f"workers_{item_name}_{report_id}", label_visibility="collapsed")
                        with col_lock:
                            if item_is_locked:
                                ...
                                if qty > 0 and not is_prep:
                                    if st.button("🔒", key=f"lock_{item_name}_{report_id}", help="鎖定此項目"):
                    ...
                    if checked:
                        with col_prep:
                            is_prep = st.checkbox("僅施工 (未完工)", value=is_prep_default, help="若尚未完成全部工項請勾選", key=f"prep_{item_name}_{report_id}")
                        with col_workers:
                            item_workers = st.number_input("投入工數", min_value=0.0, value=float(default_workers), step=0.5, key=f"workers_{item_name}_{report_id}", label_visibility="collapsed")
                        qty = float(default_qty); comp_days = 1.0
                    elif item_is_locked:
                        continue
                    else:
                        continue
                ...
                if checked or item_is_locked:
                    ...
                    selected_items.append({
                        ...
                        'revenue': revenue, 'completion_days': comp_days if 'comp_days' in dir() else 1.0,
                        'worker_count': item_workers, 'is_custom': False})
        else:
        ...
        for i in range(st.session_state[custom_key]):
            ...
            d_name = ""; d_qty = 0.0; d_unit = "式"; d_price = 0.0; d_workers = 0.0; d_remark = ""
            if i < len(custom_data_list):
                row = custom_data_list[i]
                d_name = row['項目名稱']; d_qty = row['數量']; d_unit = row['單位']
                d_price = row['單價']; d_workers = row['計工']; d_remark = row.get('備註', '')
            with cc1:
                if has_predefined_items:
                    options = [""] + extra_items_list + ["自訂..."]
                    default_idx = 0
                    if d_name in extra_items_list: default_idx = options.index(d_name)
                    elif d_name and d_name not in extra_items_list: default_idx = len(options) - 1
                    selected_item = st.selectbox(f"選擇工項", options, index=default_idx, key=f"c_select_{i}_{report_id}", label_visibility="collapsed")
                    if selected_item == "自訂...":
                        c_name = st.text_input("自訂項目名稱", value=d_name if d_name not in extra_items_list else "", key=f"c_custom_name_{i}_{report_id}", placeholder="輸入自訂項目")
            ...
            if current_role == 'admin':
                with cc2: c_qty = st.number_input(f"數量", value=float(d_qty), min_value=0.0, step=0.5, key=f"c_qty_{i}_{report_id}")
                with cc3: c_unit = st.text_input(f"單位", value=d_unit, key=f"c_unit_{i}_{report_id}")
                with cc4: c_price = st.number_input(f"單價", value=float(d_price), min_value=0.0, step=100.0, key=f"c_price_{i}_{report_id}")
                with cc5: c_workers = st.number_input(f"計工", value=float(d_workers), min_value=0.0, step=0.5, key=f"c_workers_{i}_{report_id}")
                with cc6: c_remark = st.text_input(f"備註", value=d_remark, key=f"c_remark_{i}_{report_id}", placeholder="例如：試水、抽水")
            else:
                with cc6:
                    c_remark = st.text_input(f"備註", value=d_remark, key=f"c_remark_{i}_{report_id}", placeholder="例如：試水、抽水")
                    c_qty = 0.0; c_price = 0.0; c_unit = "式"
                with cc5: c_workers = st.number_input(f"投入工數", value=float(d_workers), min_value=0.0, step=0.5, key=f"c_workers_{i}_{report_id}")
            if c_name:
            ...
            for i in range(0, len(materials), cols_per_row):
                ...
                for j in range(cols_per_row):
                    if i + j < len(materials):
                        ...
                        with cols[j]:
                            mat_checked = st.checkbox(f"{mat_name}", value=default_mat_checked, key=f"mat_{mat_name}_{report_id if report_id else 'new'}")
                            if mat_checked:
                                if current_role == 'admin':
                                    mat_qty = st.number_input(f"數量 ({mat_unit})", min_value=0.0, value=float(default_mat_qty), step=0.5, key=f"mat_qty_{mat_name}_{report_id if report_id else 'new'}")
                                else:
                                    mat_qty = 0.0; st.caption(f"已記錄 ({mat_unit})")
                                selected_materials.append({'material_name': mat_name, 'quantity': mat_qty, 'unit': mat_unit, 'unit_price': mat_price, 'cost': mat_qty * mat_price})
        else:
            st.warning("此案場尚未設定材料")
        
        st.write("---")
        uploaded_files = st.file_uploader("上傳施工照片", accept_multiple_files=True, type=['png', 'jpg', 'jpeg'], help="可上傳多張照片", key=f"photos_{report_id if report_id else 'new'}")
            ...
            if not workers.strip():
                ...
                if uploaded_files:
                    for file in uploaded_files:
                        photo_list.append({'name': file.name, 'data': file.read()})
                ...
                report_data = {
                    ...
                    'accumulated_prior_cost': total_accumulated_prior
                }
    ...
    if st.session_state.role == 'admin':
        ...
        with tab1:
                ...
                for idx, row in my_reports.head(10).iterrows():
                    with st.expander(f"{row['日期']} - {row['案場']} {row['棟別']} {row['樓層']}"):
                        ...
                        with col_info:
                            ...
                            if row['更新時間']: st.caption(f"最後更新：{row['更新時間']}")
                        with col_edit:
            ...
            if is_editing:
                ...
                if st.button("取消修改"):
                    del st.session_state.editing_report; st.rerun()
        ...
        with tab2:
            ...
            with col_f1: filter_project_view = st.selectbox("案場", ["全部"] + project_names, key="filter_view")
            with col_f2:
                ...
                else: filter_building_view = "全部"
            with col_f3: date_from = st.date_input("開始日期", None, key="date_from")
            with col_f4: date_to = st.date_input("結束日期", None, key="date_to")
            ...
            if not df.empty:
                st.write("---")
                st.write("整體績效")
                total_revenue = df['產值'].sum(); total_labor = df['人力成本'].sum()
                total_material = df['材料成本'].sum(); total_cost = df['總成本'].sum()
                total_profit = df['利潤'].sum(); total_workers = df['工數'].sum()
                total_quantity = df['總數量'].sum()
                avg_efficiency = total_quantity / total_workers if total_workers > 0 else 0
                
                col_m1, col_m2, col_m3, col_m4 = st.columns(4)
                col_m1.metric("累計產值", f"${total_revenue:,.0f}"); col_m2.metric("人力成本", f"${total_labor:,.0f}")
                col_m3.metric("材料成本", f"${total_material:,.0f}"); col_m4.metric("累計利潤", f"${total_profit:,.0f}")
                col_m5, col_m6, col_m7, col_m8 = st.columns(4)
                col_m5.metric("總成本", f"${total_cost:,.0f}"); col_m6.metric("總工數", f"{total_workers:,.1f}")
                col_m7.metric("平均工率", f"{avg_efficiency:.2f}", help="總完成數量 / 總工數")
                ...
                st.caption("數量 = 該樓層固定量（取最大值）；工數 = 實際投入累計")
                
                floor_stats_query = '''SELECT r.building_name, r.floor_name, ri.item_name, ri.unit,
                    MAX(ri.quantity) as max_qty, SUM(ri.worker_count) as total_workers,
                    MAX(ri.unit_price) as unit_price,
                    SUM(CASE WHEN ri.quantity = 0 AND ri.worker_count > 0 THEN 1 ELSE 0 END) as in_progress_count,
                    ri.is_custom
                    FROM reports r JOIN report_items ri ON r.report_id = ri.report_id WHERE 1=1'''
                floor_params = []
                    ...
                    for building in floor_df['棟別'].unique():
                        ...
                        with st.expander(f"🏢 {building} 統計明細", expanded=True):
                            for floor in building_data['樓層'].unique():
                                ...
                                st.markdown(f"**{floor}**")
                                st.dataframe(floor_item_data[['工項', '數量(MAX)', '單位', '累計工數', '累計產值', '項目利潤(僅扣工)', '狀態']],
                                    column_config={"累計產值": st.column_config.NumberColumn(format="$%d"),
                ...
                for (project, building), group_df in df.groupby(['案場', '棟別']):
                    ...
                    for idx, row in group_df.iterrows():
                        report_id = row['id']
                        with st.expander(f"{row['日期']} | {row['樓層']} | {row['人員']} | 利潤: ${row['利潤']:,.0f}", expanded=False):
                            with st.form(key=f"edit_form_{report_id}"):
                                st.markdown("1. 基本資訊")
                                col_e1, col_e2, col_e3 = st.columns(3)
                                with col_e1: new_date = st.date_input("日期", datetime.strptime(row['日期'], "%Y-%m-%d"))
                                with col_e2: new_floor = st.text_input("樓層", row['樓層'])
                                with col_e3: new_workers = st.text_input("施工人員", row['人員'])
                                new_desc = st.text_area("施工描述", row['施工描述'], height=100)
                                st.markdown("2. 完成項目 (可直接修改數值)")
                                items_df = load_report_items(report_id)
                                display_items_df = items_df.drop(columns=['完成天數']) if '完成天數' in items_df.columns else items_df.copy()
                                edited_items_df = st.data_editor(display_items_df,
                                    column_config={"產值": st.column_config.NumberColumn(disabled=True, help="自動計算"),
                                ...
                                edited_materials_df = st.data_editor(materials_df,
                                    column_config={"成本": st.column_config.NumberColumn(disabled=True, help="自動計算")},
                                ...
                                if row['照片數'] > 0:
                                    ...
                                    if photos:
                                        ...
                                        for i, photo in enumerate(photos):
                                            with p_cols[i % 3]: st.image(photo['data'], caption=photo['name'], use_container_width=True)
                                st.write("---")
                                col_btn1, col_btn2 = st.columns([1, 4])
                                with col_btn1: delete_submitted = st.form_submit_button("刪除此日報", type="secondary")
                                with col_btn2: save_submitted = st.form_submit_button("保存修改並重新計算", type="primary", use_container_width=True)
                                ...
                                if save_submitted:
                                    ...
                                    for _, item_row in edited_items_df.iterrows():
                                        ...
                                        rev = q * p
                                        wc = float(item_row['計工']) if item_row['計工'] else 0
                                        new_items_list.append({
                                            'item_name': item_row['項目名稱'], 'quantity': q, 'unit': item_row['單位'],
                                    ...
                                    for _, mat_row in edited_materials_df.iterrows():
                                        ...
                                        c_val = q * p
                                        new_materials_list.append({'material_name': mat_row['材料名稱'], 'quantity': q, 'unit': mat_row['單位'], 'unit_price': p, 'cost': c_val})
                                    ...
                                    update_data = {
                                        ...
                                        'building_name': building, 'floor_name': new_floor, 'workers': new_workers,
                                        'worker_count': actual_worker_count, 'labor_cost': labor_cost,
                                ...
                                if delete_submitted:
                                    delete_report(report_id); st.warning("已刪除該筆資料"); st.rerun()
                
                st.write("---")
                st.write("匯出資料")
                csv = df.to_csv(index=False).encode('utf-8-sig')
                st.download_button(label="下載完整報表（CSV）", data=csv,
                    file_name=f"工程日報_{datetime.now().strftime('%Y%m%d')}.csv", mime="text/csv")
                st.write("趨勢圖表")
                ...
                if st.button("清空所有日報資料", type="secondary"):
                    if st.checkbox("確認要刪除所有資料？此動作無法復原！"):
                        delete_all_reports(); st.success("已清空所有資料"); st.rerun()
            else:
                st.info("尚無符合條件的日報表")
        ...
        with tab_analysis:
            ...
            with col_a1: ana_project = st.selectbox("選擇案場", project_names, key="ana_proj")
            with col_a2:
                if ana_project:
            ...
            with col_a4:
                if ana_project:
                    c = conn.cursor()
                    item_query = 'SELECT DISTINCT ri.item_name FROM report_items ri JOIN reports r ON ri.report_id = r.report_id WHERE r.project_name = ?'
                    item_params = [ana_project]
                    if ana_bldg != "全部": item_query += " AND r.building_name = ?"; item_params.append(ana_bldg)
                    if ana_floor != "全部": item_query += " AND r.floor_name LIKE ?"; item_params.append(f"%{ana_floor}%")
            ...
            if ana_project:
                query = '''SELECT r.floor_name, ri.item_name, ri.unit, MAX(ri.quantity) as max_qty,
                    SUM(ri.worker_count) as total_workers, MAX(ri.unit_price) as unit_price,
                    COUNT(DISTINCT r.date) as work_days
                    FROM report_items ri JOIN reports r ON ri.report_id = r.report_id WHERE r.project_name = ?'''
                params = [ana_project]
                if ana_bldg != "全部": query += " AND r.building_name = ?"; params.append(ana_bldg)
                if ana_floor != "全部": query += " AND r.floor_name LIKE ?"; params.append(f"%{ana_floor}%")
                if ana_item != "全部": query += " AND ri.item_name = ?"; params.append(ana_item)
                query += " GROUP BY r.floor_name, ri.item_name ORDER BY r.floor_name, ri.item_name"
                c = conn.cursor(); c.execute(query, params); ana_data = c.fetchall()
                ...
                if ana_data:
                    ...
                    ana_df['狀態'] = ana_df.apply(lambda x: "✅ 已完成" if x['數量'] > 0 else "🔄 施工中", axis=1)
                    st.dataframe(ana_df[['樓層', '項目名稱', '單位', '數量', '累計工數', '累計產值', '預估人力成本', '項目利潤', '利潤率', '平均工率', '施工天數', '狀態']],
                        column_config={
                    ...
                    total_proj_rev = ana_df['累計產值'].sum(); total_proj_cost = ana_df['預估人力成本'].sum()
        ...
        with tab3:
            ...
            with st.expander("新增案場", expanded=False):
                with st.form("create_project_form"):
                    ...
                    copy_from = st.selectbox("選擇要複製的案場", project_names) if copy_option and project_names else None
            ...
            if project_names:
                ...
                if selected_manage_project:
                    ...
                    if new_wage != current_wage:
                        if st.button("更新工資"):
                            update_project_wage(selected_manage_project, new_wage); st.success("工資已更新"); st.rerun()
                    st.write("---")
                    ...
                    with tab_building:
                        ...
                        with st.expander("新增棟別樓層 / 批量新增", expanded=True):
                            ...
                            with col_b1: new_building = st.text_input("棟別名稱", placeholder="例如：A棟")
                            add_mode = st.radio("模式", ["單一樓層", "批量樓層 (如 3F-15F)"], horizontal=True)
                            if add_mode == "單一樓層":
                                with col_b2: new_floor = st.text_input("樓層名稱", placeholder="例如：1F")
                                if st.button("新增棟別樓層"):
                                    if new_building and new_floor:
                                        if add_building_floor(selected_manage_project, new_building, new_floor):
                                            st.success(f"已新增：{new_building} {new_floor}"); st.rerun()
                                        else: st.error("此棟別樓層已存在")
                                    else: st.error("請填寫完整資訊")
                            else:
                                with col_b2:
                                    col_start, col_end = st.columns(2)
                                    with col_start: start_floor = st.number_input("起始樓層", min_value=1, value=2, step=1)
                                    with col_end: end_floor = st.number_input("結束樓層", min_value=1, value=10, step=1)
                                existing_floors = get_building_floors(selected_manage_project, new_building) if new_building else []
                                copy_source = st.selectbox("複製項目自 (可選)", ["無"] + existing_floors)
                                if st.button("批量新增"):
                                    if new_building and start_floor <= end_floor:
                                        count = batch_add_floors(selected_manage_project, new_building, int(start_floor), int(end_floor), copy_source if copy_source != "無" else None)
                                        if count > 0: st.success(f"已成功新增 {count} 個樓層"); st.rerun()
                                        else: st.warning("沒有樓層被新增 (可能已存在)")
                                    else: st.error("請檢查輸入資訊")
                        st.write("---")
                            ...
                            for building in buildings:
                                with st.expander(f"{building}", expanded=False):
                                    ...
                                    for floor in floors:
                                        ...
                                        if locked: st.caption(f"🔒 已鎖定項目：{', '.join(locked)}")
                                        ...
                                        if floor_items:
                                            ...
                                            st.write("刪除項目")
                                            item_to_delete = st.selectbox("選擇要刪除的項目", [item[0] for item in floor_items], key=f"del_item_select_{building}_{floor}")
                                            if st.button(f"刪除項目：{item_to_delete}", key=f"del_item_btn_{building}_{floor}"):
                                                delete_floor_item(selected_manage_project, building, floor, item_to_delete); st.success(f"已刪除項目：{item_to_delete}"); st.rerun()
                                        else: st.info("此樓層尚無項目")
                                        with st.form(f"add_item_{building}_{floor}"):
                                            st.write("新增項目")
                                            col_i1, col_i2, col_i3, col_i4 = st.columns(4)
                                            with col_i1: item_name = st.text_input("項目名稱", key=f"item_name_{building}_{floor}")
                                            with col_i2: item_qty = st.number_input("標準數量", min_value=0.0, step=0.5, key=f"item_qty_{building}_{floor}")
                                            with col_i3: item_unit = st.text_input("單位", key=f"item_unit_{building}_{floor}")
                                            with col_i4: item_price = st.number_input("單價", min_value=0, step=50, key=f"item_price_{building}_{floor}")
                                            if st.form_submit_button("新增"):
                                                if item_name and item_qty > 0 and item_unit and item_price > 0:
                                                    if add_floor_item(selected_manage_project, building, floor, item_name, item_qty, item_unit, item_price):
                                                        st.success(f"已新增項目：{item_name}"); st.rerun()
                                                    else: st.error("此項目已存在")
                                                else: st.error("請填寫完整資訊")
                                        if st.button(f"刪除 {building} {floor}", key=f"del_floor_{building}_{floor}"):
                                            delete_building_floor(selected_manage_project, building, floor); st.success(f"已刪除 {building} {floor}"); st.rerun()
                                        st.write("---")
                        else: st.info("此案場尚未設定棟別樓層")
                    with tab_material:
                        st.write("材料設定")
                            ...
                            if other_projects:
                                ...
                                if st.button("匯入材料"):
                                    if copy_materials_from_project(selected_manage_project, source_proj):
                                        st.success(f"已從 {source_proj} 匯入材料"); st.rerun()
                                    else: st.error("匯入失敗")
                            else: st.info("沒有其他案場可供匯入")
                        with st.expander("新增材料"):
                            col_m1, col_m2, col_m3 = st.columns(3)
                            with col_m1: new_mat_name = st.text_input("材料名稱", placeholder="例如：500")
                            with col_m2: new_mat_unit = st.text_input("單位", placeholder="例如：組")
                            with col_m3: new_mat_price = st.number_input("單價", min_value=0, step=50, key="new_mat_price")
                            if st.button("新增材料"):
                                if new_mat_name and new_mat_unit and new_mat_price > 0:
                                    if add_material(selected_manage_project, new_mat_name, new_mat_unit, new_mat_price):
                                        st.success(f"已新增材料：{new_mat_name}"); st.rerun()
                                    else: st.error("此材料已存在")
                                else: st.error("請填寫完整資訊")
                        st.write("---")
                        ...
                        if materials:
                            ...
                            for mat_name, mat_unit, mat_price in materials:
                                ...
                                with col_m1: st.write(f"{mat_name}")
                                with col_m2: st.write(f"{mat_unit}")
                                with col_m3: st.write(f"${mat_price:,.0f}")
                                with col_m4:
                                    if st.button("刪除", key=f"del_mat_{mat_name}"):
                                        delete_material(selected_manage_project, mat_name); st.success(f"已刪除材料：{mat_name}"); st.rerun()
                        else: st.info("此案場尚未設定材料")
                    with tab_extra_work:
                        st.write("額外工項設定")
                            ...
                            if st.button("新增工項", key="add_extra_work_btn"):
                                if new_extra_item:
                                    if add_extra_work_item(selected_manage_project, new_extra_item):
                                        st.success(f"已新增工項：{new_extra_item}"); st.rerun()
                                    else: st.error("此工項已存在")
                                else: st.error("請輸入工項名稱")
                        st.write("---")
                        ...
                        if extra_items:
                            ...
                            for item_name in extra_items:
                                ...
                                with col_e1: st.write(f"{item_name}")
                                with col_e2:
                                    if st.button("刪除", key=f"del_extra_{item_name}"):
                                        delete_extra_work_item(selected_manage_project, item_name); st.success(f"已刪除工項：{item_name}"); st.rerun()
                        else: st.info("此案場尚未設定額外工項")
                    st.write("---")
                    st.write("危險操作")
                    delete_confirm_key = f"delete_confirm_{selected_manage_project}"
                    if delete_confirm_key not in st.session_state: st.session_state[delete_confirm_key] = False
                    if st.button(f"刪除案場：{selected_manage_project}", type="secondary"):
                        st.session_state[delete_confirm_key] = True; st.rerun()
                    if st.session_state[delete_confirm_key]:
                        ...
                        with col_del1:
                            if st.button("確認刪除", type="primary", key=f"confirm_del_{selected_manage_project}"):
                                delete_project(selected_manage_project); st.session_state[delete_confirm_key] = False; st.success(f"已刪除案場：{selected_manage_project}"); st.rerun()
                        with col_del2:
                            if st.button("取消", key=f"cancel_del_{selected_manage_project}"):
                                st.session_state[delete_confirm_key] = False; st.rerun()
            else: st.info("尚無案場，請先新增案場")
        ...
        with tab4:
            ...
            with st.expander("新增用戶"):
                with st.form("create_user_form"):
                    ...
                    with col_u1: new_username = st.text_input("帳號")
                    with col_u2: new_password = st.text_input("密碼", type="password")
                    with col_u3: new_role = st.selectbox("角色", ["user", "admin"])
                    if st.form_submit_button("新增", use_container_width=True):
                        if new_username and new_password:
                            if create_user(new_username, new_password, new_role):
                                st.success(f"已新增用戶：{new_username} ({new_role})"); st.rerun()
                            else: st.error("帳號已存在")
                        else: st.error("請填寫完整資訊")
            st.write("---")
            st.write("用戶列表")
                ...
                with col_u4:
                    if username != st.session_state.username:
                        if st.button("刪除", key=f"del_user_{user_id}"):
                            delete_user(user_id); st.success(f"已刪除用戶：{username}"); st.rerun()
                    else: st.write("（本人）")
            st.write("---")
            st.write("修改密碼")
            with st.form("change_password_form"):
                ...
                if st.form_submit_button("修改密碼", use_container_width=True):
                    if new_pwd: change_password(target_user, new_pwd); st.success(f"已修改 {target_user} 的密碼")
                    else: st.error("請輸入新密碼")
